...
for lp in manual:
  ...
  f = "out/RR-newstest2018-"+l+"-seg-nohy-cor.csv"
  F = open(f,'w')
  ...
  for metric in met_names[lp]:
    ...
    for sid in manual[lp]:
	  ...
	  if score1 > score2:
          ...
          #if metric in ['ITER'] and lp in ['tr-en', 'zh-en']:
            #print metric+" "+lp+" "+sid+" " + better+" "+str(score1)+" "+worse+" "+str(score2) + " " + str(conc) + " " + str(disc)

    conc = float(conc)
    disc = float(disc)
    #result = abs((conc-disc)/(conc+disc))
    result = (conc-disc)/(conc+disc)
    s = s + metric+" "+str(result)+" "+str(conc+disc)+"\n"
