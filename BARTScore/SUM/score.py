            ...
            for hypo_transform in args.hypo_transform.split(','):
                if hypo_transform.endswith('[seed]'):
                    ...
                    for seed_now in seed_lis:
                        ...
                        del res_d[mn][sn]
                    reduce_sn = 'ref' + '-' + hypo_transform + '-seedreduce'
...
class Scorer:
    ...
    def load_from_cnndm_ori(self):
        test_fn = '../plm_ft/cnndm_saves/test.json'
        data_files = {
            'test': test_fn,
        }
...
def main():
    ...
    if args.hypo_transform == 'flu-all':
        args.hypo_transform = ('flu-truncate-0.10,flu-lemmatizeverb,flu-removepreposition-0.7,flu-removestopwords-0.25,flu-noisepunct,' +
            'randomworddrop-0.10,flu-randomlocalswap-0.10,flu-randomtokenrep-0.10,flu-sentencemiddleswap,flu-removearticle')
        logger.info('flu-all detected, setting hypo_transform to %s', args.hypo_transform)
    ...
    for me in res_d:
        ...
        refa_score = res_d[me]['ref']
        for sn in res_d[me]:
            s_now = res_d[me][sn]['mean'] if isinstance(res_d[me][sn], dict) else res_d[me][sn]
            logger.info('%s: %f ref-percentage: %f', sn, s_now, (s_now - refa_score) / abs(refa_score) * 100.0)
